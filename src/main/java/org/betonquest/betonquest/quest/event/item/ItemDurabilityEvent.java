...
public class ItemDurabilityEvent implements Event {
    ...
    public void execute(final Profile profile) {
        ...
        if (itemStack.getType().isAir() || !(itemStack.getItemMeta() instanceof Damageable)) {
            return;
        }
    ...
    private void processDamage(final Player player, final ItemStack itemStack, final Damageable damageable, final double value) {
        ...
        if (damageDifference < 0) {
            durabilityModifiedDamage = -getDurabilityModifiedDamage((ItemMeta) damageable, -damageDifference, isArmor);
